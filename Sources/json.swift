...
open class JSON {
    ...
    /// NSJSONSerialization
    public init(_ obj:Any) { self._value = JSON.unwrap(obj) as AnyObject }
...
extension JSON {
    ...
    public subscript(key:String)->JSON {
        ...
        case let dic as NSDictionary:
            if let val:AnyObject = dic.object(forKey: key) as AnyObject? { return JSON(val) }
            return JSON(NSError(
                domain:"JSONErrorDomain", code:404, userInfo:[
                    NSLocalizedDescriptionKey:
                    "[\"\(key)\"] not found"
    ...
    public func makeIterator()->AnyIterator<(Any,JSON)> {
        ...
        case let o as NSDictionary:
            ...
            return AnyIterator {
                ...
                let k = ks.removeLast()
                return (k, JSON(o.value(forKey: k as! String)!))
            }
        default:
